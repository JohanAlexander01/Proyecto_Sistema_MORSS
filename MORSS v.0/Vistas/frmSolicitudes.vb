...
Public Class frmSolicitudes
    ...
    Private Sub frmSolicitudes_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        RestringirSoloNumeros(txtDni)
        RestringirSoloNumeros(txtCelular)
        MostrarSolicitudesEnGrid()
        CargarSolicitudes()
    ...
    Public Sub CargarSolicitudesConResultados()
        Try
            Dim func As New Datos.fSolicitudes
    ...
    Private Sub dgAtencion_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgAtencion.CellContentClick

    End Sub

    Public Sub RestringirSoloNumeros(txt As Guna.UI2.WinForms.Guna2TextBox)
        ' Validar escritura (KeyPress)
        AddHandler txt.KeyPress, Sub(sender, e)
        ...
        ' Validar texto pegado (TextChanged)
        AddHandler txt.TextChanged, Sub(sender2, e2)
                                        Dim campo As Guna.UI2.WinForms.Guna2TextBox = CType(sender2, Guna.UI2.WinForms.Guna2TextBox)
                                        Dim textoLimpio As String = New String(campo.Text.Where(Function(c) Char.IsDigit(c)).ToArray())

                                        If campo.Text <> textoLimpio Then
                                            Dim cursorPos As Integer = campo.SelectionStart
                                            campo.Text = textoLimpio
                                            campo.SelectionStart = Math.Min(cursorPos, campo.Text.Length)
                                        End If
                                    End Sub
    End Sub
End Class